...
with tab2:
    # ESCÁNER REPOTENCIADO
    foto = st.camera_input("📷 ESCANEAR CÓDIGO (APUNTE Y DISPARE)")
    ...
    if foto:
        if pyzbar:
            ...
            if decoded:
                ...
                if res_b.data:
                    p = res_b.data[0]
                    st.session_state.f.update({"nom": p['nombre'], "cbs": float(p['costo_bs']), "cusd": float(p['costo_usd']), "mar": float(p['margen']), "vbs": float(p['venta_bs']), "vusd": float(p['venta_usd'])})
                st.success(f"¡Código Detectado!: {codigo}")
                st.rerun()
        else:
            st.error("Librería de escaneo no disponible. Verifica requirements.txt y packages.txt")

    # BUSCADOR Y FORMULARIO (TU LÓGICA DE INVENTARIO.PY)
